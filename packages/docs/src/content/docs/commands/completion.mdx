...
- **zsh, flag and enum completion**: in the same session, exercise the
  parts most likely to regress when `_arguments` declarations are edited:
  - `gh vibe list --<TAB>` lists `--json`, `--stale`, `--limit`,
    `--allow-no-default-branch`.
  - `gh vibe clean --state <TAB>` offers the three values
    `merged`, `closed`, `merged,closed` (the composite value is the one
    most likely to break under future refactors).
  - `gh vibe completion --shell <TAB>` offers **every** value in
...
- **fish, --shell enum on completion**: `gh vibe completion --shell <TAB>`
  must offer every value in `COMPLETION_SUPPORTED_SHELLS` (regression guard
  for the enum diverging from the canonical list).
